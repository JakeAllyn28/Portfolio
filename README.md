# Jake Allyn – Simulation & Physics Engineer

Physics + software engineer specializing in real-time simulation, vehicle dynamics, and aerospace modeling.

* C++, Python, C#, MATLAB
* Unity, Godot, Unreal
* JSBSim, QGIS, LISA FEM
* Real-time physics, orbital mechanics, Monte Carlo systems
* AAA gameplay systems + government simulation work

[Resume](./JakeAllynResume.pdf) | [LinkedIn](https://www.linkedin.com/in/jake-allyn/)

---

## Overview

I build simulation systems that balance **physical accuracy, performance, and usability**.
My work spans:

* Real-time gameplay physics (AAA game engines)
* Aerospace and orbital simulation
* Experiment-driven modeling and validation
* Large-scale statistical simulation pipelines

---

## Experience & Projects

---

### Visual Concepts (NBA 2K Series)

**Focus:** Real-time vehicle physics, gameplay systems, performance

#### Featured Work

* [Vehicle Physics System](./projects/visual-concepts/vehicle-physics.md)
* [Suspension & Traction Modeling](./projects/visual-concepts/suspension.md)
* [Input System Refactor](./projects/visual-concepts/input-system.md)
* [IK Optimization & Performance Work](./projects/visual-concepts/ik-optimization.md)

#### Summary

Designed and implemented a modular, arcade-style vehicle physics system supporting multiple vehicle types (tanks, cars, aircraft, hovercraft).
Focused on responsiveness, tuning flexibility, and cross-platform performance.

---

### e-bliss LLC

**Focus:** Orbital mechanics, satellite simulation, system architecture

#### Featured Work

* [Satellite Mission Simulation Engine](./projects/e-bliss/orbital-sim.md)
* [Constellation & Communication Modeling](./projects/e-bliss/constellation.md)
* [Line-of-Sight & Coverage Analysis](./projects/e-bliss/los-analysis.md)
* [Godot Visualization Tools](./projects/e-bliss/visualization.md)

#### Summary

Architected a modular C++ simulation engine for satellite mission modeling, including orbital propagation, communication links, and scenario analysis tools.

---

### ACTA-SH

**Focus:** Physics research, blast modeling, experimental validation

#### Featured Work

* [Blast & Debris Simulation Models](./projects/acta/blast-modeling.md)
* [Monte Carlo Risk Analysis Pipeline](./projects/acta/monte-carlo.md)
* [Impulse Sled System](./projects/acta/impulse-sled.md)
* [FAA Aircraft Failure Simulation](./projects/acta/faa-sim.md)

#### Summary

Developed physics-based models validated against real-world experiments.
Built large-scale simulation workflows for risk analysis and collaborated with FAA/USAF on aircraft failure modeling.

---

### Personal Projects

**Focus:** Exploration, prototyping, independent systems

#### Featured Work

* [Digital Twin / Vehicle Simulation Concepts](./projects/personal/digital-twin.md)
* [Unity Vehicle Systems](./projects/personal/unity-vehicles.md)
* [AR + Image Scanning Systems](./projects/personal/ar-scanning.md)
* [Procedural World Generation](./projects/personal/procedural-world.md)

---

## Technical Approach

I focus on building systems that:

* Balance realism with responsiveness and usability
* Are modular and extensible for long-term iteration
* Support rapid tuning and iteration by non-programmers
* Scale across simulation fidelity levels (gameplay ↔ high-fidelity modeling)

---

## Skills

**Languages:**
C++, Python, C#, MATLAB

**Tools / Engines:**
Unity, Godot, JSBSim, QGIS

**Domains:**

* Physics Simulation
* Orbital Mechanics
* Vehicle Dynamics
* Monte Carlo / Statistical Modeling
* Real-Time Systems

---

## Planned Additions

This portfolio is actively being expanded. Upcoming sections:

* Detailed system breakdowns (with diagrams)
* Performance profiling case studies
* Simulation architecture deep dives
* Interactive demos / videos

---

## Contact

* Email: [jake@allynfamily.com](mailto:jake@allynfamily.com)
* LinkedIn: [link]

---
